...
std::vector<int> searchByName(std::vector<Member> *&members) {
    ...
    if (instancesFound.size() == 0) instancesFound.push_back(-1);  // push back -1 which means empty
    for(int i = 0; i < instancesFound.size(); i++) {
        for(int j = 0; j < instancesFound.size(); i++) {
            if(instancesFound[i] == instancesFound[j]) instancesFound.erase(instancesFound.begin() + j);
        }
    }
